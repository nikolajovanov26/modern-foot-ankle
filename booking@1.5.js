var locationID; var doctorID; var officeID; var activeDiv; var impLoc; var impDoc;
...
function dataDoctor(doctorID, activeTab) {
    ...
    if (document.querySelectorAll('.show[data-type="doctor"]').length > 1) {
        document.querySelector('[data-id="' + doctorID + '"]').click()
    } else {
...
function prevButton() {
    document.querySelectorAll('[data-btn="prev"]').forEach(el => {
        el.addEventListener('click', function() {
            ...
            if (typeof history.back !== 'undefined' && activeTab !== 'iframe') {
                ...
                changeTab();
                document.querySelector('[data-tab="' + prevTab + '"]').style.display = 'block';
            }
        });
    });
}
